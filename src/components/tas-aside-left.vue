  ...
  <div class="aside aside-left d-flex aside-fixed radius-aside-secondary" id="kt_aside">
    <div class="aside-primary d-flex flex-column align-items-center flex-row-auto bg-primary">
        ...
        <ul v-if="$store.state.modules.length" class="nav flex-column w-100" role="tablist">
          <li v-for="(v,i) in $store.state.modules" :key="i" class="nav-item mb-5">
            <div @click="openSecondary(v.routes, v.name, v)" class="nav-link p-0 pointer" :class="{ 'active': v.name === openedPrimary }">
              <span class="nav-tag ml-2 animate__animated" :class="v.name === openedPrimary ? 'bg-white animate__fadeInLeft' : 'bg-transparent animate__fadeOutLeft'" style="height:80px;width:6px;border-radius:18px"></span>
              <i v-if="v.icon" :class="v.icon" style="font-size: 2.3em;" class="text-white mx-auto pr-5 font-weight-light" v-b-tooltip.right.viewport :title="v.name.split('-').map((kebab) => kebab.toLowerCase().charAt(0).toUpperCase() + kebab.slice(1)).join(' ')"></i>
      ...
      <div class="aside-footer d-flex flex-column align-items-center flex-column-auto py-4 py-lg-10">
        ...
        <a @click="reloadModule()" class="btn btn-icon btn-clean btn-hover-primary btn-lg mb-1 position-relative" id="kt_quick_panel_toggle" v-b-tooltip.right title="Update Website">
          <span class="svg-icon svg-icon-custom svg-icon-white p-1">
