  ...
  <div class="login login-2 login-signin-on d-flex flex-column flex-lg-row flex-column-fluid bg-white" id="kt_login">
    ...
    <div class="login-aside order-2 order-lg-1 d-flex flex-row-auto position-relative overflow-hidden bg-white" style="box-shadow: 8px 0px 12px rgba(0, 0, 0, 0.34);border-radius: 0px 48px 48px 0px;z-index:2;">
      ...
      <div class="d-flex flex-column-fluid flex-column justify-content-between py-9 px-7 py-lg-13 px-lg-30" style="border-radius: 0px 48px 48px 0px;">
        <!--begin::Logo-->
        <router-link :to="{ path: '/login' }" class="d-flex flex-column align-items-center pt-20">
            ...
            <b-overlay :show="isLoading" rounded="sm">
              <form @submit="loginState($event)" class="form fv-plugins-bootstrap fv-plugins-framework" novalidate="novalidate" id="kt_login_signin_form">
                ...
                <div class="text-center pt-0">
                  ...
                  <button  :disabled="!captcha"  id="kt_login_signin_submit" class="pointer btn btn-primary font-weight-bolder font-size-h6 px-8 py-4 my-3">Masuk</button>
                </div>
              </form>
            </b-overlay>
            <!--end::Form-->
...
export default {
  ...
  mounted () {
    // <!-- 6LeLrgYlAAAAAGfHpG5y7_j4uB10T2r3-0O3CGn_ -->
    // eslint-disable-next-line no-undef
    grecaptcha.render('g-recaptcha', {
      sitekey: '6LeLrgYlAAAAANwc7t55m4MujWE2Ot0Nmvl1JkDJ',
      callback: ($event) => {
        this.captcha = $event
      },
      'expired-callback': ($event) => {
        this.captcha = null
      },
    ...
    // eslint-disable-next-line no-undef
    const result = recaptchaCallback()
    console.log(result)
  ...
  methods: {
    loginState (e) {
      ...
      if (this.username && this.password) {
        let data = { username: this.username, password: this.password, device: 'web' }
        if (this.$_config.options.sso) data.app_code = this.$_config.code
        this.isLoading = true
        this.$_api[this.$_config.options.sso ? 'sso' : 'post']('/login', data)
          .then((res) => {
    ...
    async saveProfile (e) {
      ...
      if (e.roles) this.$store.commit('set', ['roles', e.roles, true])
      await this.$store.dispatch('setModule')
      this.$router.push({ name: 'main-module' })
    },
    forgetPassword () {
      this.$_api
        .post('/request-forgot-password', { email: this.forgetEmail })
        .then((res) => {
