<template>
  <div>
    ...
    <div class="separator separator-dashed mt-8 mb-5"></div>
    <b-modal id="modal-profile" title="Profil Pengguna" hide-footer hide-header no-close-on-backdrop size="lg">
      <b-row>
        <b-col class="d-flex align-items-center justify-content-between">
          <h5 class="card-title mb-0">
            <span an class="card-label font-weight-bolder text-dark">Perbarui Informasi profil Anda</span>
          </h5>
          <a @click="$bvModal.hide('modal-profile')" class="btn btn-xs btn-icon btn-light btn-hover-primary" aria-expanded="true" aria-controls="profile-module" style="overflow-anchor: none;">
        ...
        <ValidationObserver v-slot="{ handleSubmit  }" ref="formProfileObserver">
          <form autocomplete="off" @submit.prevent="handleSubmit(changeProfile)">
            <div class="row">
              ...
              <div class="col">
                <s-input v-model="formProfile.fullname" :item="{ label:'Nama Pengguna', type:'text', validation:['required', 'min:6'], placeholder: '', api:'' }" />
                <s-input v-model="formProfile.username" :item="{ label:'Username', type:'text', validation:['required', 'min:5'], placeholder: '', api:'' }" />
              </div>
            </div>
            <div class="row">
              <div class="col-12">
              ...
              <div class="col-12">
                <s-input v-model="formProfile.email" :item="{ label:'Email', type:'text', validation:['required','email'], placeholder: '', api:'' }" />
              </div>
             <!-- <div class="col-6">
        ...
        <ValidationObserver v-slot="{ handleSubmit  }" ref="formPasswordObserver">
          <form autocomplete="off" @submit.prevent="handleSubmit(changePassword)">
            <div class="row">
              ...
              <div class="col-12">
                <ValidationProvider rules="required" v-slot="{ errors, classes }" vid="password" :name="'Kata sandi Lama'">
                  <div class="form-group">
                    <label for="">Kata sandi Lama</label>
                    <input v-model="formPassword.old_password" :type="formPassword.show? 'text': 'password' " class="form-control" :class="classes">
                    <div class="invalid-feedback">{{ errors[0] }}</div>
                  </div>
                </ValidationProvider>
              </div>
              <div class="col-6">
                <ValidationProvider rules="required|min:6" v-slot="{ errors, classes }" vid="password" :name="'Kata sandi Baru'">
                  <div class="form-group">
                    <label for="">Kata sandi Baru</label>
                    <input v-model="formPassword.new_password" :type="formPassword.show? 'text': 'password' " class="form-control" :class="classes">
                    <div class="invalid-feedback">{{ errors[0] }}</div>
              ...
              <div class="col-6">
                <ValidationProvider rules="confirmed:password|required|min:6" v-slot="{ errors, classes }" :name="'Konfirmasi Kata sandi'">
                  <div class="form-group">
...
export default {
  ...
  computed: {
    ...
    //   }
    //   return arrayColumn(this.roles, 'role_name')
    // }
  },
  methods: {
    logOut () {
