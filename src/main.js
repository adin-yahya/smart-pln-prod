...
Vue.use(VueGoogleMaps, { load: { region: 'ID', language: 'id', key: 'AIzaSyArJkzKNSGOGAwtMcsCl6cRlFfAG_dIqmE', libraries: 'places,geometry', v: 'beta' }, installComponents: true })
Vue.use(Geocoder, {
  ...
  created () {
    ...
    if (code && code === this.$_config.code) {
      Object.keys(items).forEach(e => {
        if (e !== 'loglevel:webpack-dev-server' && e !== '_grecaptcha' && items[e] !== 'undefined') this.$store.commit('set', [e, JSON.parse(items[e]), false])
      })
    } else {
      localStorage.clear()
