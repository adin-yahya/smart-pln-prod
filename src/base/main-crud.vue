  ...
  <div :class="[slave ? 'container-fluid p-0 mt-5' : classes || 'container']">
    <template v-if="!isLoading">
      <div id="create" ref="create" class="animated" v-if="activeView === 'create'">
        <slot name="create-form">
          <tas-base-form :slave="slave" @back="config.redirectAfterForm ? onSuccessForm('create', null) : getEvent('list')" @success="onSuccessForm('create', $event)" :key="config.title + '-form-create'" :form="'create'" :API="{ setter: configAPI.create, getter: configAPI.read }" :title="config.title" :fields="fields.create">
            ...
            <template v-slot:form-footer="rowData">
              <slot name="form-footer" v-bind:rowData="rowData.rowData"></slot>
            </template>
          </tas-base-form>
        </slot>
      </div>
      ...
      <div id="detail" ref="detail" class="animated" v-if="activeView === 'detail'">
        ...
        <tas-base-detail :hideBack="hideBack" :hideDetailAction="hideDetailAction" :isBodySlot="!!$scopedSlots['detail-body']" :isSideSlot="!!$scopedSlots['detail-side-right-body'] || !!$scopedSlots['detail-side-left-body']" :isCustomAPI="config.custom_api ? !!config.custom_api.read : false" @zoom="getEvent('zoom', $event)" @back="getEvent('list')" @update="getEvent('update', $event)" :key="config.title + '-detail'" :id="activeID" :slave="config.slave" :permission="permission" :title="config.title" :deleteAPI="configAPI.delete" :API="configAPI.read" :fields="fields.detail" :export="config.export">
          <template v-slot:detail-body>
            <slot name="detail-body" v-bind:id="activeID"></slot>
          </template>
          <template v-slot:detail-body-header="rowData">
            <slot name="detail-body-header" v-bind:rowData="rowData.rowData"></slot>
          </template>
      ...
      <div id="list" ref="list" class="animated" v-if="activeView === 'list'">
        ...
        <slot name="list-body">
          ...
          <tas-base-card v-else @zoom="getEvent('zoom', $event)" @add_new="getEvent('create')" @filter="getEvent('filter')" @export="getEvent('export')" @import="getEvent('import')" @detail="getEvent('detail', $event)" @update="getEvent('update', $event)" @delete="getEvent('delete', $event)" :key="config.title + '-card'" :slave="slave" :permission="permission" :title="config.title" :API="configAPI.list" :endpoint="configAPI" :fields="fields.list" :export="config.export" :import="config.import" :filters="filters">
            <template v-if="!advanceFilter" v-slot:inline-filter>
              <tas-base-form class="animated" btnSaveLabel="Terapkan Filter" v-show="filterView" @back="getEvent('list')" @success="onSuccessForm('filter', $event)" :key="config.title + '-inline-form-filter'" simple :form="'filter'" :API="{ setter: configAPI.create, getter: configAPI.read }" :title="config.title" :fields="fields.filter" />
            </template>
            <template v-slot:table-action>
    ...
    <b-modal id="zoom-modal" hide-footer hide-header centered size="lg">
      <div class="close-preview">
        <button type="button" @click="$bvModal.hide('zoom-modal')" aria-label=" Close" class="mb-1 btn btn-sm btn-icon btn-light btn-hover-primary" style="overflow-anchor: none;"><i class="ki ki-close icon-sm text-muted"></i></button>
      </div>
            ...
            this.generateFieldAPI(res).then((result) => {
              this.buildField(config.fields, result, config.pk_field).then((result) => {
                const sortFields = (array, sortArray) => {
                  ...
                  // =================
                  // Minus (-) => urutan lokal diutamakan
                  // Plus (+)  => urutan gen-model diutamakan
                  // =================
                }
                const localSort = config.fields.map(x => x.id)
                result.update = sortFields(result.update, localSort)
                result.create = sortFields(result.create, localSort)
                result.detail = sortFields(result.detail, localSort)
                result.filter = sortFields(result.filter, localSort)

                result.list = this.swapPrimaryField(sortFields(result.list, localSort), config.pk_field)
                this.fields = result
          ...
          if (item.methods.create) {
            if (typeof item.methods.create === 'object') {
              create = {
                ...
                transform: item.methods.create.transform,
          ...
          if (item.methods.update) {
            if (typeof item.methods.update === 'object') {
              update = {
                ...
                transform: item.methods.update.transform,
          ...
          if (item.methods.filter) {
            if (typeof item.methods.filter === 'object') {
              filter = {
                ...
                transform: item.methods.filter.transform,
