          ...
          <div v-if="!isLoading" class="card card-custom">
            <div class="card-header border-0 pt-5" style="min-height:0">
              ...
              <div class="card-toolbar my-0">
                <template v-if="!hideDetailAction">
                  <a @click="deleteData(realData)" v-if="$_sys.isAllowed(permission.delete)" class="btn btn-light-danger btn-icon btn-circle mr-3" v-b-tooltip.top="$t('global.delete')">
                    <i class="ri-delete-bin-2-line"></i>
                  </a>
                  <a @click="emitEvent('update', realData)" v-if="$_sys.isAllowed(permission.update)" class="btn btn-light-warning btn-icon btn-circle mr-3" v-b-tooltip.top="$t('global.update')">
                    <i class="ri-edit-2-line"></i>
                  </a>
                  <div v-if="$_sys.isAllowed(permission.delete) || $_sys.isAllowed(permission.update)" class="v-separator mr-3"></div>
                </template>
                <a v-if="!$store.state.unknownRoute && !hideBack" @click="emitEvent('back')" class="btn btn-outline-primary btn-icon btn-circle" v-b-tooltip.top="$t('global.go_back')">
                  <i class="ri-arrow-go-back-line p-0"></i>
                </a>
              </div>
            </div>
            <div class="card-body py-2 mt-3">
              <slot name="detail-body-header" v-bind:rowData="realData" />
                        ...
                        <template v-else>
                          ...
                          <template v-else>
                            ...
                            <template v-else>
                              ...
                              <template v-else-if="mainData[h.id].type === 'link'">
                                <a v-if="mainData[h.id].data" style="word-break: break-all;" class="text-primary" :class="[classReader(mainData[h.id].data, h.class)]" :href="mainData[h.id].data" target="_blank">
                                  Lihat File
                                  <i class="text-primary ri-arrow-right-up-line ml-1"></i>
                                </a>
                              </template>
                              <template v-else-if="mainData[h.id].type === 'map'">
                                  <map-input :readonly="true" v-model="mainData[h.id].data" :multiple="false" />
                              </template>
                              <template v-else-if="mainData[h.id].type === 'file'">
                                <template v-if="realData[h.id]">
                                  ...
                                  <template v-else-if="supportDisplayInt.includes(realData[h.id].ext)">
                                    <div class="d-flex  justify-content-between">
                                      <a @click="toggleIframeFile(mainData[h.id])" class="text-primary font-style-underline pointer">{{ showIframe.includes(mainData[h.id]) ? 'Sembunyikan' : 'Lihat' }} {{ realData[h.id].filename }}</a>
                                      <a :href="mainData[h.id].data" download target="_blank" class="label label-info label-inline pointer">Download</a>
                                    </div>
...
export default {
  ...
  props: {
    ...
    deleteAPI: { type: String, required: true },
    ...
    deleteData (data) {
      this.$_alert.confirm('Hapus Permanen', 'Data yang sudah dihapus tidak dapat dikembalikan, lanjutkan ?').then((result) => {
        if (result.isConfirmed) {
          this.$_api
            .delete(this.deleteAPI, { data: data })
            .then((res) => {
              this.emitEvent('back')
