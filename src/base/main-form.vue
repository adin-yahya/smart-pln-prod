<template>
  <section>
            ...
            <div :class="'row'">
              <template v-for="(fl, ix) in fields.filter((x) => $_sys.isAllowed(x.permission))">
                <div v-if="fl.show" :class="simple ? ['col-auto', fl.class] : fl.class ? fl.class : 'col-12'" :key="ix + '-loop-col'">
                  ...
                  <template v-else-if="inputGroup.includes(fl.type)">
                    <div class="form-group" :key="ix + '-field'" v-if="fl.show">
                      <ValidationProvider mode="eager" :name="fl.label" :rules="fl.validation.join('|')" v-slot="{ classes, errors }">
                        <label v-if="fl.type != 'hidden'">
                          {{ fl.label }}
                        ...
                        <div class="input-group mb-3">
                          <div class="input-group-prepend" v-if="fl.prepend">
                            <span class="input-group-text">{{ fl.prepend }}</span>
                          </div>
                          <input v-on="fl.event" v-bind="fl.attr" v-model.lazy.trim="modelData[fl.data]" :type="fl.type" class="form-control" :class="[fl.validation.length === 0 ? '' : classes]" :placeholder="fl.placeholder" />
                          <div class="input-group-append" v-if="fl.append">
                            <span class="input-group-text">{{ fl.append }}</span>
                          </div>
                        </div>
                        <div class="invalid-feedback">{{ errors[0] }}</div>
                      </ValidationProvider>
                          ...
                          <div v-if="tmp_option[fl.id]" :class="[fl.validation.length === 0 ? '' : classes, fl.class ? fl.class : fl.type + '-inline']" class="mt-2">
                            <template v-for="(o, i) in tmp_option[fl.id]">
                              <label :class="fl.type" :key="i" :for="fl.data + '-' + i" :id="'label-' + fl.data + '-' + i" class="mr-5">
                                ...
                                <span></span>
                                <a :id="'text-' + fl.data + '-' + i">{{ o.label ? o.label : o | parse(fl.transform) }}</a>
                              </label>
                              <b-popover v-if="o.tooltip" :key="i + '-option-tooltip'" :target="'text-' + fl.data + '-' + i" triggers="hover" :placement="'right'" :boundary="'window'">
                                <template #title>Keterangan {{ o.label ? o.label : o | parse(fl.transform) }}</template>
              ...
              <template v-if="simple">
                <div class="col-12 align-self-center">
                  ...
                  <button v-if="form === 'filter'" type="button" class="btn btn-secondary" @click="resetForm()">Reset Filter</button>
                </div>
              </template>
            </div>
    ...
    <slot name="form-footer" v-bind:rowData="modelData"></slot>
  </section>
</template>
<style>
...
export default {
  ...
  methods: {
    async loadForm () {
      ...
      await this.checkDefaultValue()
      if (this.form === 'update') await this.getValueAPI()
      await this.buildParamOption()
      this.taskCount++
    },
    buildModelData () {
    ...
    setWatcherToggle (target, toggle, owner) {
      this.$watch(
        ...
        function (t, f) {
          ...
          if (meetCondition) {
            if (toggle.item === 'value') this.$set(this.modelData, [target.data], toggle.toggle[0] === 'defaultData' ? (this.defaultData ? this.defaultData[target.data] || null : null) : toggle.toggle[0])
            else {
            ...
            if (toggle.item === 'value') this.$set(this.modelData, [target.data], toggle.toggle[1] === 'defaultData' ? (this.defaultData ? this.defaultData[target.data] || null : null) : toggle.toggle[1])
            else {
      ...
      this.$watch(
        ...
        function (t, f) {
          if (owner.getter) {
            ...
            if (this.lookupType.includes(owner.type)) {
              if (this.taskCount) this.$set(this.modelData, owner.data, null) // reset owner value
