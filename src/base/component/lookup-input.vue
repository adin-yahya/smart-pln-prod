<template>
  <div class="lookup-wrapper">
    ...
    <b-modal ref="lookup-modal" id="lookup-modal" :centered="true" :hide-footer="true" :hide-header="true" :size="'xl'" no-close-on-backdrop @hide="$emit('input', revertValue(modelData))">
      <!-- <h3 class="mb-3">{{ placeholder || multiple? $t('global.select_multiple') : $t('global.select_single') }} {{ title }}</h3> -->
      <div class="row px-4 d-flex align-items-center justify-content-between">
        <h3 class="mb-3">{{ title }}</h3>
        <button type="button" @click="closeReset()" aria-label=" Close" class="mb-3 btn btn-xs btn-icon btn-light btn-hover-primary" style="overflow-anchor: none;"><i class="ki ki-close icon-xs text-muted"></i></button>
      </div>
      <div class="row">
        <div :class="multiple? 'col-lg-8': 'col-lg-12'" class="mb-lg-0 mb-md-3">
          ...
          <template v-if="dataList.length && !isLoading">
          ...
          <template v-else-if="!(dataList.length) && !isLoading">
            <div class="text-center mt-3 " style="margin-bottom:3em;">
...
export default {
  ...
  computed: {
    selectedLabel: function () {
      ...
      if (this.multiple) {
        ...
        else return data.map(x => { return { [this.pointer.label]: x[this.pointer.label], [this.pointer.code]: x[this.pointer.code] } })
      } else {
    ...
    caption () {
      ...
      if (this.multiple) {
        return selected.length === 0 ? this.$t('global.select_multiple') : selected.length + ' Terpilih'
      } else return selected || this.defaultCaption || this.$t('global.select_single')
    }
  },
  watch: {
    param: {
  ...
  methods: {
    ...
    convertValue (data, fromList = false) {
    ...
    async getList (localParam = null) {
      ...
      await this.$_api.dataset(this.endpoint, newParam).then(async res => {
    ...
    closeReset () {
      if (this.multiple) this.modelData = []
      else this.modelData = {}
      this.$bvModal.hide('lookup-modal')
    }
  }
}
</script>
