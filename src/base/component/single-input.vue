<template>
  <div class="form-group" :class="item.suffix || item.prefix || item.nm ? item.groupClass || 'mb-0': item.groupClass || ''">
    <ValidationProvider mode="eager" :name="item.label ?item.label: item.prefix" :rules="item.validation.join('|')" v-slot="{ classes, errors }" :class="[item.suffix || item.prefix ? 'row' : '', item.rowclass]">
      ...
      <label :class="item.classLabel? item.classLabel: 'col-auto'" class="col-form-label" v-if="item.prefix">{{ item.prefix }} <span v-if="item.validation.includes('required') && !item.label" class="text-danger">*</span> </label>
      ...
      <div class="invalid-feedback" v-if="(typeof item.showErrorMsg !== 'undefined' && item.showErrorMsg) || typeof item.showErrorMsg == 'undefined'">{{ errors[0] }}</div>
    </ValidationProvider>
  </div>
</template>
