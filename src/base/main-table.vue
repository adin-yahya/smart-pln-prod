<template>
  <div>
    <div class="card card-custom" :class="!slave ? 'mb-2' : 'mb-1'">
      <div class="card-header border-0 py-4 pr-3 pl-5">
        ...
        <div class="card-toolbar align-items-end">
          ...
          <a v-if="this.import" v-b-modal.modal-import class="btn btn-light-primary btn-circle mr-2" v-b-tooltip.top="$t('global.import')">
        ...
        <b-toast id="export-toast" variant="success" solid no-auto-hide no-close-button :visible="isLoadingExport">
          ...
          File Excel akan langsung terunduh ketika proses Ekspor data ke Excel selesai
        </b-toast>
      </div>
    </div>
    ...
    <b-modal id="modal-import" no-close-on-backdrop hide-footer :title="'Import Data ' + title">
      <ol class="ml-0 pl-3">
        <li>
          <div class="mb-3">
            <span class="font-size-lg font-weight-bolder">Download Template</span>
            <p>File ini memiliki kolom heading sesuai dengan data yang diperlukan untuk mengimport data dengan benar</p>
            <span @click="downloadImport()" class="pointer text-info font-weight-bold">Download Template Import {{ title }}</span>
          </div>
        </li>
        <li>
          <div class="mb-3">
            <span class="font-size-lg font-weight-bolder">Pengisian Data</span>
            <p>
              Isi file Excel sesuai instruksi yang ada di
              <b>Sheet LEGEND</b>
            </p>
          </div>
        </li>
        <li>
          <div>
            <span class="font-size-lg font-weight-bolder">Upload Template</span>
            <p>Upload Template untuk dilakukan pengecekan oleh sistem. Klik tombol dibawah ini untuk mengupload template excel {{ title }}</p>
            <div class="d-flex align-items-center">
              <div class="col-auto pl-0">
                <button @click="$refs.inputFileImport.click()" :disabled="isLoadingImport" class="btn btn-light-success mb-0">Upload Template</button>
                <input ref="inputFileImport" type="file" hidden @change="uploadImport($event)" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel">
              </div>
              <div v-if="isLoadingImport" class="col pr-0">
                <span class="font-size-sm text-dark-75">Upload Progress</span>
                <b-progress class="mb-1" :value="uploadPercentage" :max="100" variant="success" animated></b-progress>
              </div>
            </div>
          </div>
        </li>
...
export default {
  ...
  computed: {
    ...
    urlProperties () {
      if (!this.slave) {
        return { ...this.paramAPI, ...this.$route.query }
  ...
  methods: {
    ...
    downloadImport () {
      ...
      let filename = 'Template Import - ' + this.title
      this.$_api.downloadFile(this.import.templateURL, null, filename + '.xlsx').then(res => {
    ...
    uploadImport (e) {
      ...
      // if ((e.target.files[0].size / 1000000).toFixed(2) < this.uploadMax) {
      this.isLoadingImport = true
      let file = e.target.files[0]
      let formData = new FormData()
      // let form = JSON.parse(JSON.stringify(this.cleanFilter))
      // for (let i = 0; i < Object.keys(form).length; i++) {
      //   const e = Object.keys(form)[i]
      //   formData.append(e, form[e])
      // }
      formData.append('file', file)
      this.$_api.uploadFile(this.import.uploadURL, formData, event => {
